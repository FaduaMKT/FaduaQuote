...
import { ArrowLeft, Eye, Pencil, Archive, UserPlus, X, RefreshCw } from "lucide-react";
...
function VendedoresContent() {
  ...
  useEffect(() => {
    ...
    // Load initial data from localStorage, fallback to JSON
    const stored = localStorage.getItem("fadua_vendedores");
    if (stored) {
      try {
        setAllVendedores(JSON.parse(stored));
      } catch (e) {
        setAllVendedores(data.vendedores);
      }
    } else {
      setAllVendedores(data.vendedores);
      localStorage.setItem("fadua_vendedores", JSON.stringify(data.vendedores));
    }
  }, [router]);
  ...
  const handleArchiveMerge = (id: string, archive: boolean) => {
              ...
              <form onSubmit={handleSaveModal} className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#888] font-semibold mt-1">
                    Si se deja vacío o no carga, se generará un avatar por defecto.
                  </p>
                </div>
